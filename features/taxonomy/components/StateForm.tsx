...
import { createState, updateState } from '@/features/taxonomy/actions'
...
export function StateForm({ open, onClose, state }: StateFormProps) {
        ...
        if (open) {
            if (state) {
                ...
                setForm({
                    ...
                    meta_robots: state.meta_robots ?? 'index,follow',
                    h1_override: state.h1_override ?? '',
                    intro_html: state.intro_html ?? '',
                })
                setSlugTouched(true)
            } else {
                setForm({
                    slug: '', name: '', abbr: '', sort_order: 0,
    ...
    function handleSubmit() {
        ...
        startTransition(async () => {
            ...
            const result = isEditing
                ? await updateState(state!.slug, payload)
    ...
    return (
        <Modal open={open} onClose={onClose} title={isEditing ? 'Edit State' : 'Add State'}>
            <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        <label className="mb-1 block text-sm font-medium">Abbreviation</label>
                        <Input
                            value={form.abbr}
                            onChange={e => setForm(prev => ({ ...prev, abbr: e.target.value.toUpperCase().slice(0, 2) }))}
                            placeholder="e.g. UP"
                            maxLength={2}
                        />
                    </div>
                ...
                <div className="flex items-center gap-2">
                    ...
                    />
                    <label htmlFor="state-active" className="text-sm font-medium">Active</label>
                </div>
