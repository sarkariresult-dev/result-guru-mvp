                        ...
                        <div className="flex flex-col gap-1.5 pr-8 min-w-0">
                             <div className="flex items-start justify-between gap-3">
                                ...
                                <div className="flex shrink-0 items-center gap-1.5 mt-0.5">
                                    {isNew && (
                                        <span className="inline-flex items-center rounded-full bg-amber-100 px-1.5 py-0.5 text-[9px] font-black uppercase tracking-tighter text-amber-700 dark:bg-amber-900/30 dark:text-amber-400 ring-1 ring-inset ring-amber-200/50 dark:ring-amber-800/20">
                                            New
                                        </span>
                                    )}
                                    {isClosingSoon && (
                                        <span className="inline-flex items-center rounded-full bg-orange-100 px-1.5 py-0.5 text-[9px] font-black uppercase tracking-tighter text-orange-700 dark:bg-orange-900/30 dark:text-orange-400 ring-1 ring-inset ring-orange-200/50 dark:ring-orange-800/20">
                                            Closing Soon
                                        </span>
                                    )}
                                    {post.application_status === 'closed' && (
                                        <span className="inline-flex items-center rounded-full bg-red-100 px-1.5 py-0.5 text-[9px] font-black uppercase tracking-tighter text-red-700 dark:bg-red-900/30 dark:text-red-400 ring-1 ring-inset ring-red-200/50 dark:ring-red-800/20">
                                            Closed
                                        </span>
                                    )}
                                </div>
                            ...
                            <div className="flex flex-wrap items-center justify-between gap-2">
                                <span className="text-[10px] text-foreground-subtle font-medium uppercase tracking-widest opacity-80">
                                    {post.state_name || 'Central'}
                                </span>
                                <span className="text-[10px] text-foreground-subtle font-medium uppercase tracking-widest opacity-80">
                                    {(post.org_short_name || post.org_name) || (post.qualification && post.qualification.length > 0 ? post.qualification.slice(0, 2).join(', ') : 'Department')}
                                </span>
                            </div>
                        </div>
