...
BEGIN
  ...
  PERFORM cron.schedule('refresh_mvs', '*/15 * * * *', 'SELECT fn_refresh_materialized_views()');

  -- Weekly / Monthly cleanup jobs
  PERFORM cron.schedule('purge_old_ad_events',   '0 4 * * 0', 'SELECT fn_purge_old_ad_events_batched(90)');
