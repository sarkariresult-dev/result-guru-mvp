...
-- ── 9. Missing RLS Policies from Hardening ──────────────────────────────
DO $$ BEGIN ALTER TABLE content_clusters ENABLE ROW LEVEL SECURITY; EXCEPTION WHEN undefined_table THEN NULL; END $$;
DO $$ BEGIN ALTER TABLE post_affiliate_products ENABLE ROW LEVEL SECURITY; EXCEPTION WHEN undefined_table THEN NULL; END $$;

DROP POLICY IF EXISTS "clusters_public_read" ON content_clusters;
DO $$ BEGIN CREATE POLICY "clusters_public_read" ON content_clusters FOR SELECT USING (TRUE); EXCEPTION WHEN undefined_table THEN NULL; END $$;

DROP POLICY IF EXISTS "clusters_admin_write" ON content_clusters;
DO $$ BEGIN CREATE POLICY "clusters_admin_write" ON content_clusters FOR ALL USING (fn_is_admin()); EXCEPTION WHEN undefined_table THEN NULL; END $$;

DROP POLICY IF EXISTS "post_affiliate_public_read" ON post_affiliate_products;
DO $$ BEGIN CREATE POLICY "post_affiliate_public_read" ON post_affiliate_products FOR SELECT USING (TRUE); EXCEPTION WHEN undefined_table THEN NULL; END $$;

DROP POLICY IF EXISTS "post_affiliate_staff_write" ON post_affiliate_products;
DO $$ BEGIN CREATE POLICY "post_affiliate_staff_write" ON post_affiliate_products FOR ALL USING (fn_is_author_or_admin()); EXCEPTION WHEN undefined_table THEN NULL; END $$;
