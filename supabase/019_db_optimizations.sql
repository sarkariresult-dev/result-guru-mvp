-- ═══════════════════════════════════════════════════════════════════════════
-- 019_db_optimizations.sql - Result Guru
-- DATABASE OPTIMIZATION: D2 – D6
-- (Note: D1 materialized views have been moved to 016_materialized_views.sql)
-- ═══════════════════════════════════════════════════════════════════════════
...
-- ── D2f: Post views (Removed: indices are inherited from 013_indexes.sql) ────
...
ANALYZE posts; ANALYZE post_tags; ANALYZE tags; ANALYZE organizations; ANALYZE states; ANALYZE categories; ANALYZE subscribers;
...
BEGIN
  ...
  RETURN QUERY SELECT p.id, p.type::TEXT, CASE WHEN p.application_start_date IS NULL AND p.application_end_date IS NULL THEN 'none' WHEN p.application_start_date > CURRENT_TIMESTAMP THEN 'upcoming' WHEN p.application_end_date < CURRENT_TIMESTAMP THEN 'closed' WHEN p.application_end_date IS NOT NULL AND p.application_end_date <= CURRENT_TIMESTAMP + INTERVAL '7 days' THEN 'closing_soon' WHEN p.application_start_date <= CURRENT_TIMESTAMP AND (p.application_end_date IS NULL OR p.application_end_date > CURRENT_TIMESTAMP) THEN 'open' ELSE 'none' END AS application_status, p.title, p.slug, p.excerpt, p.state_slug, s.name AS state_name, p.org_name, p.org_short_name, c.slug AS category_slug, c.name AS category_name, p.featured_image, p.featured_image_alt, p.view_count, p.reading_time_min, p.published_at, ts_rank_cd(p.search_vector, _tsquery, 32) AS rank, _total AS total_count FROM posts p LEFT JOIN states s ON s.slug = p.state_slug LEFT JOIN categories c ON c.id = p.category_id WHERE p.search_vector @@ _tsquery AND p.status = 'published' AND (p_type IS NULL OR p.type = p_type::post_type) ORDER BY rank DESC, p.published_at DESC LIMIT p_limit OFFSET p_offset;
END;
$$;
...
RETURNS TABLE (id UUID, type TEXT, title TEXT, slug TEXT, excerpt TEXT, featured_image TEXT, featured_image_alt TEXT, org_name TEXT, published_at TIMESTAMPTZ) LANGUAGE sql STABLE SET search_path = public AS $$
...
RETURNS TABLE (type TEXT, total_count BIGINT, open_count BIGINT, latest_published TIMESTAMPTZ) LANGUAGE sql STABLE SET search_path = public AS $$
...
END;
$$;
REVOKE EXECUTE ON FUNCTION fn_purge_old_ad_events_batched(INT, INT) FROM anon, authenticated;
...
$$;
REVOKE EXECUTE ON FUNCTION fn_ensure_future_partitions() FROM anon, authenticated;
