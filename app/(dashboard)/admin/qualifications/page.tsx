...
import { getAdminQualifications } from '@/features/taxonomy/queries'
import { QualificationsClient } from '@/features/taxonomy/components/QualificationsClient'
import { PAGINATION } from '@/config/constants'
import { Search, GraduationCap, ChevronLeft, ChevronRight } from 'lucide-react'

export const metadata = {
    title: 'Qualifications | Admin | Result Guru',
    description: 'Manage educational qualification levels (10th, 12th, Graduate, etc.)',
}
...
export default async function QualificationsPage({ searchParams }: {
    ...
    const { data: qualifications, count } = await getAdminQualifications({
        page,
    ...
    function buildUrl(overrides: Record<string, string | undefined>) {
        ...
        const qs = p.toString()
        return `/admin/qualifications${qs ? `?${qs}` : ''}`
    }

    return (
        <div className="space-y-6">
            <QualificationsClient count={count} qualifications={qualifications}>
                {/* Controls row */}
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    {/* Search */}
                    <form action="/admin/qualifications" method="GET" className="relative w-full max-w-sm">
                ...
                {qualifications.length === 0 ? (
                    <div className="flex flex-col items-center gap-3 rounded-xl border border-border bg-surface px-6 py-16 text-center">
                        <GraduationCap className="size-10 text-foreground-subtle" />
                        <p className="font-medium text-foreground">
                            {search ? 'No qualifications match your search' : 'No qualifications found'}
                        </p>
                        <p className="text-sm text-foreground-muted">
                            {search ? 'Try a different search term.' : 'Create your first qualification level to get started.'}
                        </p>
                    </div>
                ) : null}
            ...
            {totalPages > 1 && (
                <nav className="flex items-center justify-between" aria-label="Pagination">
                    <p className="text-sm text-foreground-muted">
                        ...
                        <span className="ml-1 text-foreground-subtle">({count} qualifications)</span>
                    </p>
